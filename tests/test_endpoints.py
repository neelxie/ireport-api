...
class TestIncident(TestStructure):
    ...
    def test_add_redflag(self):
        ...
        # the data list in the response
        self.assertIn("data", response.data.decode())
        ...
        change_status = self.app.patch('/api/v1/red-flags/1/status', data=json.dumps(
            {'status': "resolved"}), content_type='application/json', headers=self.headers)
        ...
        delete_red_flag = self.app.delete(
            '/api/v1/red-flags/1',
            content_type='application/json',
            headers=self.headers)
