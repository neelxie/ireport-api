...
class Valid:
    ...
    def check_user_base(self, first_str, sec_str, thrd_str, fth_str):
        ...
        """
        if self.validate_string(first_str) is False or self.validate_string(sec_str) is False or self.validate_string(thrd_str) is False:
    ...
    def check_credential(self, phn_num, email, pass_word, ad_min):
        ...
        if self.validate_string(email) or not re.match(r"[^@.]+@[A-Za-z]+\.[a-z]+", email):
    ...
    def validate_login(self, user_name, password):
        ...
        """
        if self.validate_string(user_name) is False or self.validate_string(password) is False:
